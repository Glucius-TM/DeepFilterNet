...
impl Application for SpecView {
    ...
    fn new(_flags: ()) -> (Self, Command<Message>) {
        ...
        let noisy =
            SPEC_NOISY.get_or_init(|| Arc::new(Mutex::new(SpecImage::new(w, h, -100., -10.))));
        let enh = SPEC_ENH.get_or_init(|| Arc::new(Mutex::new(SpecImage::new(w, h, -100., -10.))));
        let noisy_img = noisy.lock().unwrap().image_handle();
        let enh_img = enh.lock().unwrap().image_handle();
        (
            Self {
                df_worker,
                lsnr: 0.,
    ...
    fn update(&mut self, message: Message) -> Command<Message> {
        match message {
            ...
            Message::EnhChanged => {
                self.enh_img =
                    SPEC_ENH.get().unwrap().lock().expect("Failed to lock SPEC_ENH").image_handle();
            }
...
impl SpecView {
    ...
    fn update_noisy(&mut self) -> Option<impl Future<Output = Message>> {
        ...
        Some(async move {
            let n = recv.len();
            {
                let mut spec = SPEC_NOISY.get().unwrap().lock().expect("Failed to lock SPEC_NOISY");
    ...
    fn update_enh(&mut self) -> Option<impl Future<Output = Message>> {
        ...
        Some(async move {
            let n = recv.len();
            {
                let mut spec = SPEC_ENH.get().unwrap().lock().expect("Failed to lock SPEC_ENH");
                spec.update(recv.iter().take(n), n);
            }
            Message::EnhChanged
        })
